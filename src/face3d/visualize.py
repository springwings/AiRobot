...
# draft
from src.utils.GlobalParams import MODELS
...
def gen_composed_video(args, device, first_frame_coeff, coeff_path, audio_path, save_path, exp_dim=64):
    ...
    if MODELS["FaceReconModel"] is None:
        MODELS["FaceReconModel"] = FaceReconModel(args)
    ...
    for k in tqdm(range(coeff_pred.shape[0]), 'face3d rendering:'):
        ...
        MODELS["FaceReconModel"].forward(cur_coeff_full, device)

        predicted_landmark = MODELS["FaceReconModel"].pred_lm # TODO.
        predicted_landmark = predicted_landmark.cpu().numpy().squeeze()

        rendered_img = MODELS["FaceReconModel"].pred_face
